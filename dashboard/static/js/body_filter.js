(function (L) {
    var _this = null;
    L.BodyFilter = L.BodyFilter || {};
    _this = L.BodyFilter = {
        data: {
        },

        init: function () {
            L.Common.loadConfigs("body_filter", _this, true);
            _this.initEvents();
        },

        initEvents: function(){
            var op_type = "body_filter";
        ...
        initStatisticBtnEvent:function(){
            $(document).on( "click",".statistic-btn", function(){
                ...
                window.location.href = "/body_filter/rule/statistic?rule_id="+rule_id+"&rule_name="+encodeURI(rule_name);
            });
