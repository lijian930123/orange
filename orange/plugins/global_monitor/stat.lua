...
local status = ngx.shared.global_monitor
...
function _M.count(key_suffix)
    ...
    local key = "urltable"
    local urlTable = status:get(key) or ""

    if urlTable == "" then
        urlTable = key_suffix
    else
        local fields = urlTable:split("|")
        if not isInTable(key_suffix, fields) then
            urlTable = urlTable .. "|" .. key_suffix
        end
    end

    status:set(key, urlTable)
